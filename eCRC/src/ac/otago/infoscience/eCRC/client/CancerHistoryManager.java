		...
		for (int i = 0; i < details.length; i++) {
			...
			for (int rows = 1; rows < grid.getRowCount(); rows++) {
				if (grid.getWidget(rows, 0) != null
					...
					// deceased
					boolean deceased = ((SimpleCheckBox) grid.getWidget(rows,
							Util.COL_DE_CHECKBOX)).getValue();
					
					String dAgeString = (((TextBox) grid.getWidget(rows,
							Util.COL_DE_AGE)).getValue()).toString();

					short dAge = -1;
					if (null != dAgeString && dAgeString.length() > 0) {
						dAge = Short.parseShort(dAgeString);

					}
					
					// nhi number
					String nhiNumber = thePanel.getPatientNHI();
					// comment
					String comment = thePanel.getComments();
					...
					histories[historiesIndex] = new CancerHistory(
							nhiNumber, 
							erefID,
							relationshipType, 
							name, 							 
