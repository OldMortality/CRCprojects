...
public class RiskPanel extends VerticalPanel {
	 ...
	 */
	RiskPanel(Form1 par, int riskLevel, String patientName, String patientDOB,
			String patientNHI, CancerHistory[] histories) {
		super();
		parent = par;
		...
		hidden.getElement().setAttribute("risklevel",
				new Integer(riskLevel).toString());

		// contains close and print buttons
		HorizontalPanel buttonPanel = new HorizontalPanel();

		logger.log(Level.FINE, "Starting risk calculation dialogue for "
				+ patientName + ", result= " + riskLevel);
		...
		Label detailsLabel = new Label(" Risk assessment for " + patientName
				+ ", Date of Birth: " + patientDOB + ", NHI number: "
				+ patientNHI);
		...
		Button backButton = new Button("BACK ", new ClickHandler() {
			public void onClick(ClickEvent event) {
				parent.setBackToFamilyPanel();
			}
		});

		// keep wrong id, because it may be all over the test cases
		backButton.ensureDebugId(Util.ID_BUTTON_CLOSE);
		...
		// Add a close button at the bottom of the dialog
		Button closeButton = new Button("CLOSE AND SEND TO E-REFERRAL ", new ClickHandler() {
			public void onClick(ClickEvent event) {
				parent.closeApplication();
			}
		});
		closeButton.ensureDebugId(Util.ID_BUTTON_CLOSE_ALL);
