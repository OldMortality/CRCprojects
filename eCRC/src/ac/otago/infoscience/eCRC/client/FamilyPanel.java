...
public class FamilyPanel extends AbsolutePanel implements ClickHandler {
	...
	Image familyImage = new Image("images/family.png");
	...
	CrossImage[] crossImages = new CrossImage[Util.NUMBER_OF_RELATIONSHIPS];
	...
	FamilyPanel(Form1 theForm) {
		...
		// add the cross signs, for deceased people
		for (int type = 0; type < crossImages.length; type++) {
			crossImages[type] = new CrossImage(type);
			// set invisible
			crossImages[type].setVisible(type, false);
			int x = sections.whereToPutCrossImageX(type);
			int y = sections.whereToPutCrossImageY(type);
			this.add(crossImages[type], x, y);
	...
	public void setCross(int who) {
		// TODO Auto-generated method stub
		logger.log(Level.FINEST,
				"setCross " + who + " " + Util.getDescription(who));
		crossImages[who].setVisible(who, true);
	}

	public void removeCross(int who) {
		// TODO Auto-generated method stub
		logger.log(Level.FINEST,
				"removeCancerSign " + who + " " + Util.getDescription(who));
		crossImages[who].setVisible(who, false);
	}
