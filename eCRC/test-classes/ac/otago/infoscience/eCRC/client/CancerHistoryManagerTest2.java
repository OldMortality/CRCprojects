...
public class CancerHistoryManagerTest2 extends GWTTestCase {
	...
	public void test1() {
		...
		// deceased
		SimpleCheckBox de = new SimpleCheckBox();
		de.setValue(true);
		grid.setWidget(1, Util.COL_DE_CHECKBOX, de);

		// colon cancer age
		TextBox deAge = new TextBox();
		deAge.setValue("66");
		grid.setWidget(1, Util.COL_DE_AGE, deAge);

		// comments
		TextArea comments = new TextArea();
		String COMMENT_1 = "Comment for parental grandfather";
		comments.setText(COMMENT_1);
		detailsPanel.setComments(comments);
		...
		assertEquals("run number: " + runNumber, COMMENT_1,
				history[0].getComment());
		...
		runNumber = 7;
		SimpleCheckBox dBox = (SimpleCheckBox) detailsPanel.getGrid()
				.getWidget(1, Util.COL_DE_CHECKBOX);
		dBox.setValue(false);
		manager.toHistories(details);
		history = manager.getHistories();
		...
		runNumber = 8;
		TextBox d = (TextBox) detailsPanel.getGrid().getWidget(1,
				Util.COL_DE_AGE);
		d.setValue("-1");
		manager.toHistories(details);
		history = manager.getHistories();
		...
		// deceased
		SimpleCheckBox de2a = new SimpleCheckBox();
		de2a.setValue(false);
		grid2.setWidget(1, Util.COL_DE_CHECKBOX, de2a);

		SimpleCheckBox de2b = new SimpleCheckBox();
		de2b.setValue(true);
		grid2.setWidget(2, Util.COL_DE_CHECKBOX, de2b);

		// deceased  age
		TextBox deAge2a = new TextBox();
		deAge2a.setValue("99");
		grid2.setWidget(1, Util.COL_DE_AGE, deAge2a);

		TextBox deAge2b = new TextBox();
		deAge2b.setValue("100");
		grid2.setWidget(2, Util.COL_DE_AGE, deAge2b);

		
		// comments
		TextArea comments2 = new TextArea();
		String COMMENT_2 = "Comment for final panel";
		comments2.setText(COMMENT_2);
		detailsPanel2.setComments(comments2);
		...
		assertEquals("run number: " + runNumber, COMMENT_2, history2[8].getComment());
		assertEquals("run number: " + runNumber, COMMENT_2, history2[9].getComment());
